...
class SystemDescription:
    ...
    def compute_objective_instances(self, nodes, nodeTemplates):
        if len(self.objectiveInstances) > 0:
            print "Recomputing objective instances"
            del self.objectiveInstances[:]  # Emptying existing list.

        for objective in self.objectives:
            if objective.isLocal:
                # If local objective, loop through each node category and instantiate objective instances for every
                ...
                for nodeCategory in objective.nodeCategories:
                    for node in self.get_node_names(nodeCategory, nodes, nodeTemplates):
                        ...
                        objectiveInstanceToAdd.node = node.name
                        self.objectiveInstances.append(objectiveInstanceToAdd)

            else:
                objectiveInstanceToAdd = ObjectiveInstance()
                objectiveInstanceToAdd.name = objective.name + "_obj_instance"
                objectiveInstanceToAdd.objectiveName = objective.name
                self.objectiveInstances.append(objectiveInstanceToAdd)
    ...
    # TODO: Look into updating this logic to use objective instances generated prior to this.
            ...
            for functionality in objective.functionalities:
                if functionality.name not in self.computedFunctionalities:
                    if functionality.perNode:
                        ...
                        # of the objective to get all nodes of interest.
                        for nodeCategory in objective.nodeCategories:
                            for node in self.get_node_names(nodeCategory, nodes, nodeTemplates):
                                ...
                                if node.status == "ACTIVE":
                                    ...
                                    functionalityInstanceToAdd.objectiveInstanceName = \
                                        objective.name + "_obj_instance_" + node.name
                        ...
                        functionalityInstanceToAdd.functionalityName = functionality.name
                        functionalityInstanceToAdd.objectiveInstanceName = objective.name + "_obj_instance"
                        self.functionalityInstances.append(functionalityInstanceToAdd)
                        self.computedFunctionalities.append(functionalityInstanceToAdd.functionalityName)
        else:
            for functionality in objective.functionalities:
                        ...
                        for constraint in replicationConstraints:
                            ...
                            if initialNumInstances != 0:
                                ...
                                if constraint.kind == "VOTER_REPLICATION":
                                    ...
                                    voterInstanceToAdd.objectiveInstanceName = objective.name + "_obj_instance"
                                ...
                                for i in range (0, initialNumInstances):
                                    ...
                                    if constraint.kind == "CONSENSUS_REPLICATION":
                                        ...
                                        consensusInstanceToAdd.objectiveInstanceName = objective.name + "_obj_instance"
    ...
    def check_collocation_constraint(self, functionality):
        ...
        for constraint in self.constraints:
            if functionality.name == constraint.functionalities[0] and \
                            constraint.kind == "NODE_COLLOCATION":
                retval.append(constraint)
...
class Functionality:
    name = None
    dependsOn = None
    perNode = None  # This attribute is only valid if parent objective isLocal.
...
class SolverBackend:
    ...
    def __init__(self):
        ...
        self.objectiveInstName2Index = dict()
    ...
    def add_replication_constraints(self, solver, initial):
        for constraint in self.functionalityConstraints:
            ...
            if constraint[0] == "distribute":
                ...
                objectiveInstanceIndex = self.objectiveInstName2Index[constraint[3]]
                constraintToAdd = solver.ForceAtleast(objectiveInstanceIndex, compIndexes, constraint[1])
            ...
            #        compIndexes.append(self.componentInstName2Index[componentInstanceName])
            #    objectiveInstanceIndex = self.objectiveInstName2Index[constraint[3]]
            #    constraintToAdd = solver.ForceAtmost(objectiveInstanceIndex, compIndexes, constraint[1])
            ...
            #        compIndexes.append(self.componentInstName2Index[componentInstanceName])
            #    objectiveInstanceIndex = self.objectiveInstName2Index[constraint[3]]
            #    constraintToAdd = solver.ForceExactly(objectiveInstanceIndex, compIndexes, constraint[1])
    ...
    def load_system_descriptions(self, db):
        ...
        for sd in collection.find():
            ...
            for o in systemDescription.objectives:
                ...
                objectiveToAdd.isLocal = objective.isLocal

                # Store node categories only if objective isLocal.
                if (objectiveToAdd.isLocal):
                    for nc in objective.nodeCategories:
                        objectiveToAdd.nodeCategories.append(nc)
                ...
                for f in objective.functionalities:
                    ...
                    # Store perNode flag only if objecitve isLocal.
                    if (objectiveToAdd.isLocal):
            ...
            systemDescriptionToAdd.compute_objective_instances(self.nodes, self.nodeTemplates)
            ...
            # Retrieve and store objective instances.
            for objectiveInstance in systemDescriptionToAdd.get_objective_instances():
                self.objectiveInstances.append(objectiveInstance)

            # Sort above objective instances and store name to ID mapping.
            self.objectiveInstances = sorted(self.objectiveInstances, key = attrgetter("name"))
            for i in range (len(self.objectiveInstances)):
                self.objectiveInstName2Index[self.objectiveInstances[i].name] = i
            ...
            # print
            #
            # print "=====OBJECTIVE INSTANCES====="
            # for i in self.objectiveInstances:
            #      print i.name, "(", i.objectiveName, ")"
            #
            # print
            #
            # print "=====FUNCTIONALITY INSTANCES====="
            # for i in self.functionalityInstances:
            #      print i.name, "(", i.functionalityName, ",", i.objectiveInstanceName, ")"
            #
            # print
            #
